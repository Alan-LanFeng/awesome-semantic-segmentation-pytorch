...
prediction_keys = ['obj_trajs', 'obj_trajs_mask', 'track_index_to_predict', 'obj_trajs_pos', 'obj_trajs_last_pos', 'center_objects_world', 'map_center', 'obj_trajs_future_state', 'obj_trajs_future_mask', 'center_gt_trajs', 'center_gt_trajs_mask', 'center_gt_final_valid_idx', 'center_gt_trajs_src', 'map_polylines', 'map_polylines_mask', 'map_polylines_center', 'kalman_difficulty', 'trajectory_type']
...
class MotionPredictionModelOutput(AbstractModelOutput):
    """Margin for (multiclass) image classification. See Section 3.3 of `our
    paper <https://arxiv.org/abs/2303.14186>`_ for more details.
    """
    ...
    def get_out_to_loss_grad(self, model: Module,
        weights: Iterable[Tensor],
        buffers: Iterable[Tensor],
        batch,
    ) -> Tensor:
        return torch.ones(batch[0].shape[0],1).to(batch[0].device)


class SemanticSegmentationModelOutput(AbstractModelOutput):
    """Margin for (multiclass) image classification. See Section 3.3 of `our
    paper <https://arxiv.org/abs/2303.14186>`_ for more details.
    """
    ...
    def get_output(
        ...
        outputs = ch.func.functional_call(model, (weights, buffers), image)
        loss_dict = model.criterion(outputs, label)
...
TASK_TO_MODELOUT = {
    ...
    "motion_prediction": MotionPredictionModelOutput,
    "semantic_segmentation": SemanticSegmentationModelOutput,
}
