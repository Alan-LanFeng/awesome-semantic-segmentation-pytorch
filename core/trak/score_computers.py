...
class BasicScoreComputer(AbstractScoreComputer):
    ...
    def get_xxt(self, grads: ch.Tensor) -> ch.Tensor:
        self.sample_dim = grads.shape[0]
        result = ch.zeros(
            self.sample_dim, self.sample_dim, dtype=self.dtype, device=self.device
        )
        blocks = ch.split(grads, split_size_or_sections=32, dim=1)

        for block in blocks:
            result += block.to(self.device) @ block.T.to(self.device)
    ...
    def get_x_xtx_inv(self, grads: Tensor, xtx: Tensor) -> Tensor:
        ...
        xtx_inv = ch.linalg.inv(xtx_reg.to(ch.float32).to('cpu')).to(self.device)
