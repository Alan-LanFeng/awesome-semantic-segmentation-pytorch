...
class TRAKer:
    ...
    def __init__(
        ...
        #self.normalize_factor = ch.tensor(self.num_params_for_grad, dtype=ch.float32)**0.75
    ...
    def featurize(
        ...
        # grads = grads / grads.norm(2, -1, keepdim=True).clamp_min(1e-12)
    ...
    def finalize_features(
        ...
        for model_id in tqdm(model_ids, desc="Finalizing features for all model IDs.."):
            ...
            #self.saver.current_store["dist_mat"][:] = cosine_similarity_matrix.to(self.dtype).cpu()
            if del_grads:
    ...
    def finalize_scores(
        ...
        _g_mmp = self.saver.current_store["grads"]
        _g_target_mmp = self.saver.current_store[f"{exp_name}_grads"]
        ...
        for j, model_id in enumerate(
            ...
            g_target = ch.as_tensor(
                self.saver.current_store[f"{exp_name}_grads"], device='cpu'
            )
            _g_cpu += g
        ...
        eigvals, eigvecs = torch.linalg.eig(xtx_all)
        ...
        self.scores = _scores_on_cpu
        print('finished')

        return _scores_on_cpu
    
    def get_ot_dual(self,g_candidate,g_target):
        import geomloss
        g_candidate = g_candidate.to(torch.float32)
        g_target = g_target.to(torch.float32)
        ...
        distance = distance.to(g_candidate.device)
        g_candidate = g_candidate.to('cpu')
        g_target = g_target.to('cpu')

        f,g = distance(g_candidate, g_target)
        f,g = f[0],g[0]
        N = f.shape[0]
        f = f - (sum(f)-f)/(N-1)
        print(f'negative_rate = {sum(f<0)/N}')

        distance.potentials = False

        c = distance(g_candidate, g_target)
        print(f'distance {c}')
